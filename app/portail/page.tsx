...
const Btn=({children,onClick,color=C.gold,style={}}:{children:React.ReactNode,onClick?:()=>void,color?:string,style?:React.CSSProperties})=>
  <button onClick={onClick} style={{background:color,color:color===C.gold||color===C.green?"#000":"#fff",border:"none",borderRadius:8,padding:"10px 20px",cursor:"pointer",fontWeight:700,fontSize:13,fontFamily:"inherit",width:"100%",...style}}>{children}</button>;

const BtnGhost=({children,onClick,style={}}:{children:React.ReactNode,onClick?:()=>void,style?:React.CSSProperties})=>
  <button onClick={onClick} style={{background:"transparent",color:C.muted,border:`1px solid ${C.border}`,borderRadius:8,padding:"9px 18px",cursor:"pointer",fontSize:12,fontFamily:"inherit",...style}}>{children}</button>;

const Card=({children,style={}}:{children:React.ReactNode,style?:React.CSSProperties})=>
  <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:20,...style}}>{children}</div>;

const CT=({children,style={}}:{children:React.ReactNode,style?:React.CSSProperties})=>
  <div style={{background:C.card2,border:`1px solid ${C.border}`,borderRadius:10,padding:14,...style}}>{children}</div>;

const Pill=({children,color=C.gold}:{children:React.ReactNode,color?:string})=>
  <span style={{background:color+"22",color,border:`1px solid ${color}44`,borderRadius:20,padding:"3px 10px",fontSize:11,fontWeight:600}}>{children}</span>;

const SM=({val,max,color=C.green}:{val:number,max:number,color?:string})=>
  <div style={{height:5,borderRadius:3,background:C.border,overflow:"hidden"}}>
    <div style={{height:"100%",width:`${Math.min(100,(val/max)*100)}%`,background:color,borderRadius:3}}/>
  </div>;

const fmt=(m:number)=>Number(m).toLocaleString("fr",{minimumFractionDigits:0,maximumFractionDigits:0})+" €";

// Composant séparé pour éviter useState dans un .map()
const AvisCard=({m,showToast}:{m:any,showToast:(s:string)=>void})=>{
  const[note,setNote]=useState(0);
  const[hover,setHover]=useState(0);
  const[comm,setComm]=useState("");
  const[envoye,setEnvoye]=useState(false);
  return <Card style={{borderColor:envoye?`${C.green}44`:C.border}}>
...
const Login=({onLogin}:{onLogin:(c:any)=>void})=>{
...
const PortailClient=({user,onLogout,showToast}:{user:any,onLogout:()=>void,showToast:(s:string)=>void})=>{
  ...
  return <div style={{display:"flex",minHeight:"100vh",background:C.dark,fontFamily:"'Segoe UI',system-ui,sans-serif",color:C.text}}>
    ...
    <div style={{flex:1,overflowY:"auto"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {devis.map((d,i)=><Card key={i} style={{borderColor:d.statut==="en_attente"?`${C.orange}44`:C.border}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}>
              <div><div style={{fontSize:14,fontWeight:700,marginBottom:4}}>{d.service}</div><div style={{fontSize:11,color:C.muted}}>Réf. {d.id} · {d.date}</div></div>
              <div style={{textAlign:"right"}}><div style={{fontSize:22,fontWeight:700,color:C.gold}}>{fmt(d.montant)}</div><Pill color={d.statut==="validé"?C.green:C.orange}>{d.statut==="validé"?"✓ Validé":"⏳ En attente"}</Pill></div>
            </div>
            <div style={{display:"flex",gap:8}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          {missions.map((m,i)=>{
            ...
            return <Card key={i} style={{borderColor:`${sc}33`}}>
              ...
              {m.statut==="en cours"&&<><div style={{display:"flex",justifyContent:"space-between",fontSize:11,marginBottom:6}}><span style={{color:C.muted}}>Progression de la mission</span><span style={{color:C.green,fontWeight:700}}>{m.progression}%</span></div><SM val={m.progression} max={100} color={C.green}/></>}
      ...
      {page==="avis"&&<div style={{padding:24}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          {missions.filter(m=>m.statut==="terminé").map((m,i)=><AvisCard key={i} m={m} showToast={showToast}/>)}
        </div>
      </div>}
    </div>
  </div>;
...
const PortailApporteur=({user,onLogout,showToast}:{user:any,onLogout:()=>void,showToast:(s:string)=>void})=>{
  const[page,setPage]=useState("accueil");
  const leads=[
  ...
  const contrat={taux:20,debut:"01/03/2024"};
  ...
  const statutColor:{[key:string]:string}={proposition:C.blue,qualifié:C.gold,gagné:C.green,perdu:C.muted};
  return <div style={{display:"flex",minHeight:"100vh",background:C.dark,fontFamily:"'Segoe UI',system-ui,sans-serif",color:C.text}}>
    <div style={{width:220,background:C.card,borderRight:`1px solid ${C.border}`,display:"flex",flexDirection:"column",flexShrink:0}}>
      <div style={{padding:"18px 16px 12px",borderBottom:`1px solid ${C.border}`}}>
        <div style={{fontSize:14,fontWeight:700,color:C.gold,letterSpacing:"0.1em",fontFamily:"Georgia,serif"}}>XYRA</div>
      ...
      {page==="soumettre"&&<div style={{padding:24}}>
        ...
        <Card style={{maxWidth:560}}>
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            ...
            <div style={{background:`${C.gold}11`,border:`1px solid ${C.gold}33`,borderRadius:8,padding:12,fontSize:11,color:C.text}}>💡 Rappel : votre commission est de <b style={{color:C.gold}}>{contrat.taux}%</b> du CA signé.</div>
            <Btn onClick={()=>{
        ...
        <Card style={{height:460,padding:0,overflow:"hidden"}}>
          <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
            <div style={{padding:"12px 16px",borderBottom:`1px solid ${C.border}`,background:C.card2,display:"flex",alignItems:"center",gap:10}}>
              <div style={{width:32,height:32,borderRadius:"50%",background:`${C.gold}22`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:700,color:C.gold}}>X</div>
            ...
            <div style={{flex:1,overflowY:"auto",padding:16,display:"flex",flexDirection:"column",gap:10}}>
              {msgs.map((m,i)=><div key={i} style={{display:"flex",gap:8,flexDirection:m.moi?"row-reverse":"row"}}>
                <div style={{width:28,height:28,borderRadius:"50%",background:m.moi?`${user.couleur}22`:`${C.gold}22`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:700,color:m.moi?user.couleur:C.gold,flexShrink:0}}>{m.moi?user.avatar:"X"}</div>
