...
const NAV = [
  { group:"", items:[
    { id:"accueil",       icon:"🏠", label:"Operating Center" },
  ]},
  { group:"BUSINESS", items:[
    { id:"business",      icon:"🏢", label:"MRR, ARR & Croissance" },
  ]},
  { group:"CLIENTS", items:[
    { id:"deploiement",   icon:"👥", label:"Tous les tenants" },
  ]},
  { group:"INTELLIGENCE", items:[
    { id:"admin",         icon:"🤖", label:"Analyse IA & CEO" },
  ]},
  { group:"", items:[
      ...
      <div style={{display:"flex",flexDirection:"column",gap:10}}>
        <Card>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div>
              <div style={{fontSize:13,color:C.text,fontWeight:600}}>{actifs.length} tenant{actifs.length>1?"s":""} actif{actifs.length>1?"s":""}</div>
              <div style={{fontSize:11,color:C.muted}}>{essais.length} en periode d'essai</div>
            </div>
          </div>
        </Card>
        {essaisExpires.length>0&&(
          <Card style={{borderColor:`${C.red}44`}}>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              ...
              <div>
                <div style={{fontSize:13,color:C.red,fontWeight:600}}>{essaisExpires.length} essai{essaisExpires.length>1?"s":""} expire{essaisExpires.length>1?"s":""} sans paiement</div>
                <div style={{fontSize:11,color:C.muted}}>{essaisExpires.map(t=>t.societe).join(", ")}</div>
              </div>
            </div>
          </Card>
        )}
        {essaisExpires.length===0&&(
          <Card>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              <span style={{fontSize:24}}>👍</span>
              <div style={{fontSize:13,color:C.text}}>Aucune alerte critique pour le moment.</div>
            </div>
          </Card>
        )}
      </div>
...
const PageBusinessAdmin=({showToast})=>{
  ...
  const tenants=data?.tenants||[];
  const actifs=tenants.filter(t=>t.statut==="actif");
  const PLAN_PRIX={starter:59,business:129,enterprise:249,multi_societes:499,multi_societes_pro:799,holding:1200};
  const mrr=actifs.reduce((s,t)=>s+(PLAN_PRIX[t.plan]||0),0);
  return <div style={{padding:20}}>
    <div style={{fontSize:18,fontWeight:700,color:C.text,fontFamily:"Georgia,serif",marginBottom:16}}>🏢 Business — MRR, ARR & Croissance</div>
    {loading?(
      <div style={{padding:40,textAlign:"center",color:C.muted}}>Chargement...</div>
    ):(
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>
        <KPI label="MRR" val={mrr+" €"} color={C.gold}/>
        <KPI label="ARR estime" val={(mrr*12)+" €"} color={C.green}/>
        <KPI label="Tenants actifs" val={actifs.length} color={C.blue}/>
      </div>
    )}
  </div>;
};
              ...
              return <tr key={i} style={{background:t.statut==="suspendu"?`${C.red}08`:"transparent"}}>
                ...
                <Td>
                  <div style={{display:"flex",gap:4}}>
                    ...
                    {t.statut!=="actif"&&<BtnGhost onClick={async()=>{const res=await fetch("/api/moderer-tenant",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t.id,action:"activer"})});const d=await res.json();if(d.success){showToast("✅ "+t.societe+" active");window.location.reload();}else{showToast("Erreur: "+d.error);}}} style={{fontSize:9,padding:"3px 6px",color:C.green,borderColor:`${C.green}44`}}>Activer</BtnGhost>}
                    {t.statut!=="suspendu"&&<BtnGhost onClick={async()=>{if(!confirm("Suspendre "+t.societe+" ?"))return;const res=await fetch("/api/moderer-tenant",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t.id,action:"suspendre"})});const d=await res.json();if(d.success){showToast("⏸ "+t.societe+" suspendu");window.location.reload();}else{showToast("Erreur: "+d.error);}}} style={{fontSize:9,padding:"3px 6px",color:C.orange,borderColor:`${C.orange}44`}}>Suspendre</BtnGhost>}
                    <BtnGhost onClick={async()=>{if(!confirm("SUPPRIMER definitivement "+t.societe+" ? Cette action est irreversible."))return;const res=await fetch("/api/moderer-tenant",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t.id,action:"supprimer"})});const d=await res.json();if(d.success){showToast("🗑 "+t.societe+" supprime");window.location.reload();}else{showToast("Erreur: "+d.error);}}} style={{fontSize:9,padding:"3px 6px",color:C.red,borderColor:`${C.red}44`}}>Suppr.</BtnGhost>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div>
          ...
          <div style={{fontSize:11,color:C.muted}}>
            Toutes les plateformes fonctionnent normalement.
          </div>
        </div>
...
export default function Xyra() {
  ...
  const pageMap={
    accueil:<PageOperatingCenter tenantInfo={tenantInfo} showToast={showToast}/>,
    business:<PageBusinessAdmin showToast={showToast}/>,
    deploiement:<PageDeploiement plan={plan} showToast={showToast}/>,
    admin:<PageAdmin plan={plan} showToast={showToast}/>,
    parametres:<PageParametresAdmin showToast={showToast}/>,
  };
