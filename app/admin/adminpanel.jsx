              ...
              return <tr key={i} style={{background:t.statut==="suspendu"?`${C.red}08`:"transparent"}}>
                ...
                <Td>
                  <div style={{display:"flex",gap:4}}>
                    ...
                    {t.statut==="suspendu"&&<BtnGhost onClick={async()=>{const res=await fetch("/api/moderer-tenant",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t.id,action:"reactiver"})});const d=await res.json();if(d.success){showToast("✅ "+t.societe+" reactive");window.location.reload();}else{showToast("Erreur: "+d.error);}}} style={{fontSize:9,padding:"3px 6px",color:C.green,borderColor:`${C.green}44`}}>Reactiver</BtnGhost>}
