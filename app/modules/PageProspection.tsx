...
const PageProspection=({plan, modulesActifs,showToast,profil=null,UpgradeWall,activeCompany})=>{
  ...
  if(!hasAccess(plan,"prospection",modulesActifs))return <div style={{padding:20}}><UpgradeWall page="Prospection Auto" plan={plan}/></div>;
  return <div style={{padding:20}}>
    ...
    {onglet==="sequences"&&<RelanceIAWidget showToast={showToast} profil={profil}/>}
...
// ─── RELANCEIA WIDGET — sequences email generees par Lea selon le secteur ───
const RelanceIAWidget=({showToast,profil})=>{
  const[sequences,setSequences]=useState([]);
  const[stats,setStats]=useState(null);
  const[loading,setLoading]=useState(false);
  const[genLoading,setGenLoading]=useState(false);
  const[secteurForm,setSecteurForm]=useState(profil?.label||"");
  const[contactForm,setContactForm]=useState({email:"",prenom:"",nom:"",societe:"",tel:"",sequenceId:""});
  ...
  const genererSequence=async()=>{
    if(!secteurForm.trim())return showToast("⚠️ Precisez le secteur");
    setGenLoading(true);
    try{
      const res=await fetch('/api/relance',{
        method:'POST',
        headers:{'Content-Type':'application/json'},
        body:JSON.stringify({action:'generer_sequence',secteur:secteurForm}),
      });
      const data=await res.json();
      if(data.success){showToast("✅ Lea a genere la sequence !");charger();}
      else showToast("❌ "+(data.error||"Erreur"));
    }catch(e){showToast("❌ Erreur de connexion");}
    setGenLoading(false);
  };
  ...
  return <div>
    ...
    <CT style={{marginBottom:12,borderColor:`${C.purple}44`}}>
      <STitle>🤖 Lea genere une sequence pour votre secteur</STitle>
      <div style={{display:"flex",gap:8}}>
        <input value={secteurForm} onChange={e=>setSecteurForm(e.target.value)} placeholder="Ex: Conciergerie premium" style={{flex:1,background:C.card,border:`1px solid ${C.border}`,borderRadius:5,padding:"8px 10px",color:C.text,fontSize:12,fontFamily:"inherit"}}/>
        <Btn onClick={genererSequence} disabled={genLoading}>{genLoading?"⏳ Lea redige...":"🤖 Generer"}</Btn>
      </div>
    </CT>

    {sequences.length===0&&<div style={{textAlign:"center",padding:20,color:C.muted,fontSize:11,marginBottom:12}}>Aucune sequence pour le moment — generez-en une ci-dessus.</div>}
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
        ...
        <div style={{gridColumn:"span 2"}}>
          ...
          <select value={contactForm.sequenceId} onChange={e=>setContactForm(f=>({...f,sequenceId:e.target.value}))} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:5,padding:"6px 8px",color:C.text,fontSize:11,fontFamily:"inherit",width:"100%"}}>
            ...
            {sequences.map((s,i)=><option key={i} value={s.id}>{s.name} ({s.secteur})</option>)}
          </select>
        </div>
      </div>
