...
export const conv=(m:number,de:string,ve:string)=>{const f=DEVISES.find((x:any)=>x.code===de),t=DEVISES.find((x:any)=>x.code===ve);return(!f||!t)?m:(m/f.taux)*t.taux;};
export const Tabs=({tabs,active,onChange}:{tabs:any[],active:string,onChange:(id:string)=>void})=><div style={{display:"flex",gap:4,background:C.card2,borderRadius:8,padding:4,flexWrap:"wrap"}}>{tabs.map((t:any)=><button key={t.id} onClick={()=>onChange(t.id)} style={{background:active===t.id?C.card:"transparent",color:active===t.id?C.gold:C.muted,border:active===t.id?`1px solid ${C.border}`:"1px solid transparent",borderRadius:6,padding:"5px 12px",cursor:"pointer",fontSize:12,fontFamily:"inherit",fontWeight:active===t.id?600:400,whiteSpace:"nowrap"}}>{t.label}</button>)}</div>;
export const St=({s}:{s:string})=>{const map:Record<string,[string,string]>={validé:[C.green,"✓ Validé"],signé:[C.green,"✓ Signé"],confirmé:[C.green,"✓ Confirmé"],envoyé:[C.blue,"→ Envoyé"],en_attente:[C.orange,"⏳ En attente"],actif:[C.green,"● Actif"],critique:[C.red,"⚠ Critique"],ouvert:[C.green,"Ouvert"],complet:[C.red,"Complet"],"en cours":[C.blue,"En cours"],"à faire":[C.muted,"À faire"],complété:[C.green,"✓ Complété"]};const[c,l]=map[s]||[C.muted,s];return <Pill color={c}>{l}</Pill>;};
export const inits=(nom:string)=>nom.split(" ").map((w:string)=>w[0]).join("").slice(0,2).toUpperCase();
